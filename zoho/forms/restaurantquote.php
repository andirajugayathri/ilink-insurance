...
function mapFormFields($formData) {
    return [
        ...
        'Product_Inquiry' => 'Restaurant Insurance'
    ];
}
...
function insertDataIntoDatabase($mappedData, $pdo) {
    try {
        $sql = "INSERT INTO Restaurant_Insurance(full_name, company_name, contact_number, email, coverage) 
 ...
 error_log("data mapped: " . print_r($mappedData, true));
    ...
    if (insertDataIntoDatabase($mappedData, $pdo)) {
        if (addRecordToZoho($mappedData, $pdo)) {
            ...
            $subject = "New Insurance Inquiry from Restaurant Insurance";
...
<body>
  <h2>New Insurance Inquiry for Restaurant Insurance</h2>
  <table>
